...
export function LearnPage() {
    ...
    return (<div className="flex flex-row-reverse gap-[48px] px-6">
      ...
      <FeedWrapper>
        ...
        {data.units.map((unit) => (<div key={unit.id} className="mb-10">
            <Unit id={unit.id} order={unit.order} description={unit.description} title={unit.title} lessons={unit.lessons} activeLesson={data.courseProgress?.activeLesson} activeLessonPercentage={data.lessonPercentage}/>
          </div>))}
      </FeedWrapper>
    </div>);
}
